// tpay macOS Keychain helper.
//
// A tiny, separately code-signed executable that performs ALL Keychain I/O and
// Touch ID on behalf of the main `tpay` binary. The point of the indirection:
// a Keychain item's ACL binds to the *creating app's* code identity (cdhash).
// The main `tpay` binary is rebuilt constantly, so its cdhash churns and macOS
// would prompt for the login-keychain password on every release. This helper
// changes only when this file changes, so its cdhash is stable across tpay
// releases and the OS keeps trusting it — no password prompt.
//
// Ported from solana-foundation/pay (rust/crates/keystore/src/macos/helper.swift).
// Protocol: argv command, hex on stdin/stdout, `ERROR:<msg>` on stderr, exit 1.
...
func main() {
    guard CommandLine.arguments.count >= 2 else {
        fputs("usage: tpay-helper <command> [args...]\n", stderr); exit(1)
    }
